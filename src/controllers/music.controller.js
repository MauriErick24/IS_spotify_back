...
const obtenerCancion = async (req, res) => {
  try{
    const {id} = req.params
    const result = await pool.query("SELECT * FROM cancion WHERE idcancion = $1", [id])
...
const borrarCancion = async (req, res) => {
  const {id} = req.params
  const result = await pool.query("DELETE FROM cancion WHERE idcancion = $1 RETURNING *", [id])
